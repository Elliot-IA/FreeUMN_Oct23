...
var PartDataReturned = new EventEmitter();
var index = 0;
var partCategory = null;

var nameIndex = 0;
var PartNameReturned = new EventEmitter();
var partName = null;

var addJSIsDone = new EventEmitter();
var addJsIndex = 0;
...
function configureStandby(){
    ...
    // SEBASTIAN WRITING HERE
    // THIS IS THE PLAN
    // - COPY WHAT WE DID WITH CATEGORIES FOR SERVEFREEATUMN.JS
    // - GO TO SERVE STATION AND BASICALLY DO THE SAME
    // - GO TO LOGMASTER AND DO THE SAME
    // - THIS IS DIFFERENT, GO TO NAME.JS
    app.get("/getPN", (req,res)=>{
        // three global variables made
        // - a new emmitter called PartNameFound
        // - a new index called nameIndex
        // - a new variable called partName

        //res.json({value:"test"});
        
        /*var PartNameFound = "Part Name Found" + nameIndex; // can't use the same index we did for categories, so now it's nameIndex
        PartDataReturned.on(PartNameFound,()=>{
            nameIndex = nameIndex + 1;
            console.log("Part Name Found");
            res.json({value:partName})

        });*/


         //I don't know what this is, but I'm scared to delete it

         var rawData = fs.readFileSync(path.join(__dirname,"..","station_F@UMN/automationFiles/choppingBoard/obj"+objectNum+"Data.json"));
         var jsonObject = JSON.parse(rawData);
         var partName = jsonObject.name;
         res.json({value:partName})
    ...
    app.get("/getPC", (req,res)=>{
        //res.json({value:"test"});
        
        
        console.log("getPC heard on serveFreeAtUMN.js");
        var PartCategoryFound = "Part Category Found" + index;
        PartDataReturned.on(PartCategoryFound,()=>{
            index = index + 1;
            console.log("Part Category Found");
            res.json({value:partCategory})
    ...
    app.get("/getPT", (req,res)=>{
        var rawData = fs.readFileSync(path.join(__dirname,"..","station_F@UMN/automationFiles/choppingBoard/obj"+objectNum+"Data.json"));
        var jsonObject = JSON.parse(rawData);
        var partTag = jsonObject.tags;
        res.json({value:partTag})
    });
    
    app.get("/getDes", (req,res)=>{
        var rawData = fs.readFileSync(path.join(__dirname,"..","station_F@UMN/automationFiles/choppingBoard/obj"+objectNum+"Data.json"));
        var jsonObject = JSON.parse(rawData);
        var partDes = jsonObject.description;
        res.json({value:partDes})
    });
    ...
    app.get("/getAddJSDone", function(req, res){
        
        addJSIsDone.on("add.js is DONE" + addJsIndex, ()=>{
            c("add.js is DONE" + addJsIndex+" event was emitted! Resolving getAddJSDone fetch");
            res.json({test:"123456"});
        });
...
function configureRequests(){
    ...
    stationChild.on("message",(data)=>{
        ...
        }else if(data.command == "returnedCategoryName"){
            console.log("Command to return the category name heard from stationServer.js by serveFreeAtUMN.js");
            partCategory = data.category;
            PartDataReturned.emit("Part Category Found" + index);
        }else if(data.command == "add.js is DONE"){
            console.log("Command to tell front end that add.js is done was heard from stationServer.js by serveFreeAtUMN.js");
            //PartDataReturned.emit("Part Category Found" + index);
            addJSIsDone.emit("add.js is DONE" + addJsIndex);
            addJsIndex += 1;
        }else{
